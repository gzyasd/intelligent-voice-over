...
CPU 路线最稳，但真实剧集片段会比较慢；有 NVIDIA 显卡时可以额外安装 CUDA 版 PyTorch：
...
RTX 5090 本机验收使用的是 `torch 2.11.0+cu128` / `torchaudio 2.11.0+cu128`。注意 `uv sync` 可能会根据 `pyproject.toml` 和 `uv.lock` 回到 CPU 稳定组合；跑 GPU profile 前请重新确认 `torch.cuda.is_available()` 为 `True`。

Windows 上 PyTorch 官方 CUDA wheel 源当前没有可用的 `torchcodec` Windows wheel；新版 `torchaudio` 默认读写链路可能会因此失败。项目的 Demucs 和 F5 adapter 已使用 `soundfile` 绕过该音频读写链路，并已用授权日语真实样片验证：
