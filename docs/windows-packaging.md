...
如果只需要刷新 `dist\IntelligentVoiceOver\` 目录，不希望脚本自动生成 zip，可以加：

```powershell
uv run python .\scripts\build_windows_package.py --output-dir .\dist --skip-archive
```

这种方式适合本机调试；压缩便携包可以在确认目录内容后手动完成。
...
- 本地模型权重不会被打包进程序。用户需要把模型放到 `IntelligentVoiceOver\models`，或在客户端“模型设置”页选择其他模型目录。
- 本地模型 Python 运行环境会随发布包内置，包括 `.venv` 和 `.venv-pyannote`；用户不需要另行安装 demucs、faster-whisper、F5-TTS 或 pyannote 的 Python 包。
- GPU 驱动、CUDA 驱动组件、LM Studio 或其他模型服务仍需用户在目标机器上自行准备。
...
- 本地模型 Python 运行环境已随发布包内置；GPU 驱动、CUDA 驱动组件和 LM Studio 仍需用户在目标机器上自行准备。
...
3. 用 mock profile 或真实 profile 跑一次“开始生成配音（完整流程）”。
4. 审核时间线后执行“最终导出”。
