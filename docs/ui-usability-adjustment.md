...
1. 打开客户端，先进入“模型中心”。
2. 选择模型目录，默认建议放在程序目录下的 `models/`；也可以指定其他磁盘已有模型目录。
3. 选择推荐预设，例如“本机高质量”或“LM Studio 翻译 + 本地模型”。
4. 点击“一键检查模型”。如果缺少模型，界面会提示应该放到类似 `models/tts/F5-TTS` 的具体位置。
5. 回到首页或项目库，点击“新建配音项目”。
6. 在 4 步向导中依次选择视频、内容语言、生成方案，并确认创建；可以选择“创建并开始生成”。
7. 生成时在“当前项目 -> 生成进度”查看阶段进度、当前句子进度和失败原因。
8. 生成完成后进入“时间线”审片，默认隐藏片段 ID、原始质量标记等技术列；选中句子可查看原文、中文、说话人、情绪并重生成。
9. 在“项目库”管理以往项目，打开项目文件夹或继续未完成生成。
