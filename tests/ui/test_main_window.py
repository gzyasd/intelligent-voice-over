...
def test_model_settings_saves_and_loads_http_adapter_profile(qtbot, tmp_path) -> None:
    ...
    settings._adapter_store_path = store_path
...
def test_model_settings_guides_local_model_directory_in_chinese(qtbot) -> None:
    ...
    assert settings.check_local_readiness_button.text() == "检查就绪"
    ...
    assert settings.delete_adapter_profile_button.text() == "删除选中 adapter"
    assert settings.method_combo.currentText() == "POST"
    ...
    assert settings.advanced_group.title() == "在线 API 配置（可选）"
...
def test_model_settings_warns_when_saving_adapter_without_path(qtbot) -> None:
    ...
    assert settings.model_diagnostics_list.item(0).text() == "adapter 保存失败：请填写配置 ID"
...
def test_model_settings_load_adapter_profiles_via_dialog(monkeypatch, qtbot, tmp_path) -> None:
    from ivo.ui.model_settings import ModelSettings
    ...
    result = settings.load_adapter_profiles_from_ui()

    assert result == store_path
    assert settings._adapter_store_path == store_path
...
def test_model_settings_loads_local_command_profile_summary(qtbot, tmp_path) -> None:
    ...
    assert settings.local_profile_summary_list.item(3).text() == "translation: local / target-text overrides"
