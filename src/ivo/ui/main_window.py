...
from ivo.ui.model_services_page import ModelServicesPage
from ivo.ui.scheme_management_page import SchemeManagementPage
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.create_project_button.setToolTip("新建配音项目")
        self.open_project_button = QPushButton("\u6253\u5f00\u9879\u76ee")
        self.open_project_button.setAccessibleName("打开项目")
        self.open_project_button.setToolTip("打开已有项目")
        self.local_preview_button = QPushButton(self.START_DUBBING_TEXT)
        self.local_preview_button.setAccessibleName("本地预览")
        self.local_preview_button.setToolTip("本地命令预览配音")
        self.evaluation_report_button = QPushButton("\u751f\u6210\u8bc4\u4f30\u62a5\u544a")
        self.evaluation_report_button.setAccessibleName("评估报告")
        ...
        mark_primary_button(self.local_preview_button)
        self.open_project_button.setStyleSheet("")
        mark_secondary_button(self.open_project_button)
        self.evaluation_report_button.setStyleSheet("")
        mark_secondary_button(self.evaluation_report_button)
        self.export_button.setStyleSheet("")
        mark_secondary_button(self.export_button)
        ...
        self.model_services_page = ModelServicesPage()
        self.scheme_management_page = SchemeManagementPage(store=self.model_services_page._store)
        self.scheme_management_page.jump_to_services.connect(self._jump_to_model_services)
    ...
    def _build_model_settings_shortcut_page(self) -> QWidget:
        ...
        title = QLabel("模型设置已集中到模型服务")
        mark_heading3(title)
        detail = QLabel("请在左侧进入“模型服务”配置供应商和本地模型目录，或进入“方案管理”选择整套生成方案。")
        detail.setWordWrap(True)
        detail.setObjectName("SecondaryText")
        open_model_center_button = QPushButton("打开模型服务")
        open_model_center_button.setStyleSheet("")
        mark_primary_button(open_model_center_button)
        open_model_center_button.clicked.connect(lambda: self.app_shell.set_current_page("model_services"))
        card_layout.addWidget(title)
    ...
    def open_project_wizard(self) -> DubbingProject | None:
        wizard = ProjectWizard(self, store=self.model_services_page._store)
    ...
    def request_local_preview(self) -> PipelineWorker | None:
        if self.current_project is None:
            message = "请先在首页新建项目或从项目库打开已有项目，然后再开始生成配音。"
    ...
    def _auto_save_model_settings(self) -> None:
        if self.current_project is not None:
    ...
    def _jump_to_model_services(self, stage: str) -> None:
        """Navigate to the model services page, optionally selecting a stage."""
        self.app_shell.set_current_page("model_services")
        self.model_services_page.scroll_to_stage(stage)  # type: ignore[arg-type]
    ...
    def open_export_dialog(self) -> PipelineWorker | None:
        if self.current_project is None:
            message = "请先在首页新建项目或从项目库打开已有项目，然后再导出。"
            self.progress_label.setText(message)
    ...
    def regenerate_timeline_segment(self, segment_id: str) -> SynthesisResult | None:
        try:
            ...
            self.progress_label.setText(f"\u7247\u6bb5\u751f\u6210\u5931\u8d25\uff0c\u8bf7\u68c0\u67e5\u6a21\u578b\u670d\u52a1\u914d\u7f6e\u540e\u91cd\u8bd5\u3002: {exc}")
            QMessageBox.warning(self, "\u7247\u6bb5\u751f\u6210\u5931\u8d25\uff0c\u8bf7\u68c0\u67e5\u6a21\u578b\u670d\u52a1\u914d\u7f6e\u540e\u91cd\u8bd5\u3002", str(exc))
            return None
    ...
    def handle_segment_regeneration_failed(self, message: str) -> None:
        self.progress_label.setText(f"\u7247\u6bb5\u751f\u6210\u5931\u8d25\uff0c\u8bf7\u68c0\u67e5\u6a21\u578b\u670d\u52a1\u914d\u7f6e\u540e\u91cd\u8bd5\u3002: {message}")
        self._set_timeline_regeneration_enabled(True)
        self.regenerating_segment_id = None
        QMessageBox.warning(self, "\u7247\u6bb5\u751f\u6210\u5931\u8d25\uff0c\u8bf7\u68c0\u67e5\u6a21\u578b\u670d\u52a1\u914d\u7f6e\u540e\u91cd\u8bd5\u3002", message)
    ...
    def _try_compile_scheme_adapters(self) -> CompiledPipelineAdapters | None:
        """Try to compile the currently applied scheme into pipeline adapters.

        Returns None if no scheme is applied or compilation fails.
        """
        try:
            scheme = self.scheme_management_page.current_scheme()
            if scheme is None or not scheme.bindings:
                return None
            ...
            store = self.scheme_management_page._store
            registry = ProviderRegistry()
            secret_store = SecretStore(store._dir)
            ...
            # The config store protocol used by SchemeRuntimeCompiler expects
            # a get() method that raises on missing. We wrap ProviderStore.
            class _ConfigStoreAdapter:
                def __init__(self, ps: ProviderStore) -> None:
                    self._ps = ps

                def get(self, config_id: str) -> StageProviderConfig:
                    result = self._ps.get_stage_config(config_id)
                    if result is None:
            ...
            return compiler.compile(scheme)
        except Exception as exc:
            self.run_log_panel.append_stage_message(
                "方案编译", f"编译失败: {exc}", level="error"
            )
            return None
    ...
    def _execute_local_preview(
        ...
        # Try scheme-compiled adapters first
        compiled = self._try_compile_scheme_adapters()
        if compiled is not None:
            # Scheme was compiled — use scheme adapters exclusively.
            ...
            return run_local_command_preview(
                ...
                diarization_adapter=compiled.diarization,
                translation_adapter=compiled.translation,
                tts_adapter=compiled.tts,
    ...
    def _save_model_profile_settings(self) -> None:
        ...
        self.current_project.settings.update_profiles(
            ProfileSelectionSettings(
                local_model_path=self.model_settings.local_model_path_edit.text().strip(),
