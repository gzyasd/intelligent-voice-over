...
def main() -> int:
    parser = argparse.ArgumentParser(description="TTS command adapter skeleton for CosyVoice.")
    ...
    if args.engine_command_json:
        ...
        try:
            ...
            raise SystemExit(f"CosyVoice engine command failed: {command}") from exc
        if not audio_path.is_file():
            raise SystemExit(f"CosyVoice engine did not create audio output: {audio_path}")
        write_contract(Path(args.json_out), audio_path, args.duration_ms)
        return 0

    try:
        import cosyvoice  # type: ignore[import-not-found]  # noqa: F401
    except ImportError as exc:
        raise SystemExit(
            "Missing dependency: CosyVoice. Install it from "
            "https://github.com/FunAudioLLM/CosyVoice, download the model directory, "
            "or provide --engine-command-json for your local inference script."
        ) from exc
